    ...
    public class CityService : ICityService
    {
        ...
        public async Task<OperationResponse<Guid>> CreateCityAsync(string cityName)
        {
            if (string.IsNullOrWhiteSpace(cityName) || string.IsNullOrEmpty(cityName))
                return new OperationResponse<Guid> { IsSuccess = false, Errors = $"Wiersz pusta {nameof(cityName)}" };
            ...
            if (city is not null)
                return new OperationResponse<Guid> { IsSuccess = false, Errors = "Miasto już dodane." };
